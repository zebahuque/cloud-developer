...
(async () => {
  ...
  app.get( "/cars/", (req: Request, res: Response) => {
    ...
    //filter for optional query param if make var exists
    if(make){
  ...
  app.get("/cars/:id:", (req: Request, res: Response) => {
    ...
    //if id not given, give fail status
    if(!id) {
    ...
    //if id doesn't exist, give not found status
    if(car && car.length ===0) {
  ...
  app.post("/cars/", (req: Request, res: Response) => {
    let {make, type, model, cost, is} = req.body;
